function TwoPartyVisual() {
    
    // set in clobber, used in visualize
    var data = {};

    // get peardeck google sheet as json feed
    // calls clobber and visualize after data loads
    this.loadPearDeckSheets = function(source1, source2) {

        // an array to store the two party objects 
        // inside, each party object will have an array of speeches
        var parties = new Array(2);

        // dictionary for name and title, keyed on name
        var speakers = {}; 
        
        // google sheet url converted to json api url
        var url1 = getSheetJsonUrl(source1); // sheet 1
        var url2 = getSheetJsonUrl(source2); // sheet 2
        
        // the unique keys per google sheet (in the url)
        var key1 = getDocKey(source1);
        var key2 = getDocKey(source2);
        
        // d3.json passes `json` object to `getParty` function below
        ...
        // returns the unique google sheet key
        function getDocKey(docUrl) {
            var key = docUrl.split('/d/')[1].split('/')[0];
            return key;
        }
        ...
        function getParty(json) {
            ...
            // the first sheet is party 1
            if(sheetId.indexOf(key1) > -1) {
                parties[0] = {
                    name : key1,
                    speeches : speeches
                };
            ...
            // the second sheet is party 2
            if(sheetId.indexOf(key2) > -1) {
                parties[1] = {
                    name : key2,
                    speeches : speeches
                };
            ...
            // both sheets are finished processing - clobber & visualize!
            if(++finished === 2) {
        ...
        console.log('these sources need to be published:');
        console.log(source1);
        console.log(source2);
        console.log('built these json urls assuming the docs are already published')
        console.log(url1);
        console.log(url2);
    };
    ...
    function clobber(parties, speakers, topics) {
        ...
        function topic(topic, i) {
            ...
            topic.parties = data.parties.map(function(party) {
                ...
                topic.count += count = count / party.wordCount *1e3;
                topic.count += count
                return {
                    count: count,
                    mentions: mentions
